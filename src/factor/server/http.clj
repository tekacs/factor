...
(methodical/defmethod injection/prep-key ::server [_ config _]
  (update config :port #(or % (some-> (#'aero/get-env "PORT") Long/parseLong) 9090)))

(methodical/defmethod injection/init-key ::server [_ {:keys [port handler]} _]
  (alter-var-root #'org.httpkit.client/*default-client* (fn [_] sni-client/default-client))
  (hk/run-server handler {:port port :legacy-return-value? false}))

(methodical/defmethod injection/halt-key! ::server [_ server _]
